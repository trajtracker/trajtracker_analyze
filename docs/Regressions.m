...
% <http://trajtracker.wixsite.com/trajtracker/ttrk-analyze-design our
% website>
% 
...
% 
% * _tt.reg.regress()_ runs a specific regression model 
%   on the data of a single subject. If the regression is
%   timepoint-dependent, the function will run the regression model per
%   time point.
% * Another function loops through subjects and calls _tt.reg.regress()_ per
...
% _tt.reg.getTrialDynamicMeasures()_.
% 
...
%   example, this will include in the regressions only trials with 
%   *target > 20*:
% 
...
%   regression model attempted. This field is a _tt.reg.OneRR_ object.
% * The OneRR object reflects the results of one regression model for one
%   subject - i.e., the output of a single call to _tt.reg.regress()_.
...
%   
% * The results of one predictor are stored as a _tt.reg.OnePredRR_ object.
...
% predictor. This is done via the _tt.reg.compareParams()_ function.
% 
% The output of _tt.reg.compareParams()_ is also used for plotting the
% regression results with _tt.reg.plotParamComparison()_.
% 
% To plot the results of a single subject, transform the regression results
% into the format required by _plotParamComparison()_ - this is done using
% the _tt.reg.reformatOneRRForPlots()_ function.
% 
