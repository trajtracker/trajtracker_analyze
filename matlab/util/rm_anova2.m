function stats = rm_anova2(Y,S,F1,F2,FACTNAMES,outputDfe)
%
...
dfeA = dfA*dfS;
dfeB = dfB*dfS;
dfeAB = dfTotal-dfA-dfeA-dfB-dfeB-dfAB-dfS;

% return values
stats = {'Source','SS','df','MS','F','p','eta2g','partial_eta2', 'dfe';...
         FACTNAMES{1},                      ssA,    dfA,    msA,    fA,     pA,     ssA / (ssTot-ssB-ssAB), ssA / (ssA + ssAS), dfeA;...
         FACTNAMES{2},                      ssB,    dfB,    msB,    fB,     pB,     ssB / (ssTot-ssA-ssAB), ssB / (ssB + ssBS), dfeB;...
         [FACTNAMES{1} ' x ' FACTNAMES{2}], ssAB,   dfAB,   msAB,   fAB,    pAB,    ssAB / (ssTot-ssA-ssB),  ssAB / (ssAB + ssABS), dfeAB;...
